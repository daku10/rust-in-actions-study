...
fn main() {
    let app = App::new("clock")
        ...
        .arg(
            ...
            Arg::with_name("std").short("s").long("use-standard").takes_value(true).possible_values(&["rfc2822", "rfc3339", "timestamp"]).default_value("rfc3339")
    ...
    if action == "set" {
        ...
        let err_msg = format!("nable to parse {} according to {}", t_, std);
        ...
        match os_error_code {
            Some(0) => (),
            Some(_) => eprintln!("Unable to set time time: {:?}", maybe_error),
            None => (),
        }
    }
